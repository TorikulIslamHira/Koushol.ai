...
| Decorative UI effects | `rough-notation`, `three` | `Highlighter.tsx` (brand-gold text emphasis, used on the homepage hero) and `ModernLoginSignup.tsx` (WebGL dot-grid card, brand-green/gold tinted, now the real `/login` and `/signup` UI wired to `useAuth`) are themed and live; `InteractiveGridPattern.tsx` is themed but not yet wired into a page (see that folder's README). |
