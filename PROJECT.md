...
| 9 | Certificates: issuance on module/course completion, PDF download, public verification page (`/verify/:certificateId`) | Planned. The `certificates` table and its RLS already exist (Phase 1, schema-only) — this phase is issuance logic + UI, no new table needed. |
| 10 | Course discovery & trust: catalog search/filter (title, price, category), student ratings & reviews, verified-teacher badge, per-course SEO structured data, "notify me" waitlist for unpublished courses | Planned |
| 11 | Student engagement: notes/bookmarks while reading a topic, discussion Q&A per module/topic (student asks, teacher replies), progress streaks & badges, course/platform leaderboard, practice/retake mode that doesn't affect the recorded module_progress score, study-time tracking | Planned |
